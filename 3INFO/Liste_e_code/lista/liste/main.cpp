...
void print_ric_list(node_ptr p) {
    if (!p) return;
    cout << p->content << " ";
    print_ric_list(p->next);
}

void print_rev_ric_list(node_ptr p) {
    if (!p) return;
    print_ric_list(p->next);
    cout << p->content << " ";
}

int sum_ric_elements(node_ptr p) {
    if (!p) return 0;
    return p->content + sum_ric_elements(p->next);
}
...
int main()
{
    ...
    print_ric_list(root);
    cout << endl;
    print_rev_ric_list(root);
    cout << sum_ric_elements(root) << endl;
    return 0;
